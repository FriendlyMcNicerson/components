...
/**
 * Builds SOQL query with required and optional fields to initialize.
 */
public class SoqlQueryBuilder {
    ...
    private void buildSubquery(List<String> inputStrings, StringBuilder sb) {
        ...
        sb.append(SPACE_SEPARATOR).append(FROM_CLAUSE).append(SPACE_SEPARATOR)
                .append(inputStrings.get(0).split(UNDERSCORE)[0]).append(RIGHT_PARENTHESIS);
    }
    
    /**
     * Checks whether {code}fieldName{/code} is name of custom field.
     * Custom fields end with "__c" string
     * 
     * @param fieldName
     * @return
     */
